...
const APIKey = "RGAPI-abe5c6c0-6032-49ed-807d-6cd42e38f37f";

let lastPlayed = (summoner, champion) => {
  return new Promise(async (resolve, reject) => {
    ...
    needle.get(
      ...
      async (error, response, body) => {
        ...
        let currentChampionJson = await cache.getChampionJson(
          champion,
          currentPatchVersion
        );
        ...
        if (!lastPlayedGame) {
          console.log(
            "Could not find match with that champion recently, so defaulting to 12 months..."
          );
...
let getAccountIDFromSummonerName = summonerName => {
...
let statChanges = async (req, res) => {
  let stats = await getChampionDifferences(
    req.params.champion,
    req.params.patch
  );
...
let getChampionDifferences = async (championName, patchVersion) => {
  // get the differences of that champion since the version
  let currentPatchVersion = await getCurrentPatchVersion();
  let currentChampionJson = await cache.getChampionJson(
    championName,
    currentPatchVersion
...
let getAllData = async (req, res) => {
  let allData = {};
  allData.lastPlayed = await lastPlayed(
    req.params.summoner,
    req.params.champion
  );
  console.log(allData.lastPlayed);
  let patchId = (await database.getCurrentPatchForDate(allData.lastPlayed))[0]
    .id;
  allData.lastPlayedPatch = patchId;
  longPatchId = patchId + ".1";
  console.log(longPatchId);
  let currentPatchVersion = await getCurrentPatchVersion();
  allData.baseStatDifferences = await getChampionDifferences(
    req.params.champion,
    longPatchId
  );
  let currentChampionJson = await cache.getChampionJson(
  ...
  allData.championChanges = await database.getAllChangesForChampionIdAfterDate(
    championId,
    allData.lastPlayed
  );
  allData.championChanges.forEach(patchData => {
    ...
    patchData.changes.forEach(skill => {
      Object.keys(skill).forEach(key => {
        ...
        if (
          (key.includes("COOLDOWN") || key.includes("COST")) &&
          !key.includes("COST REFUND")
        ) {
          skill[key].isBuff =
            parseInt(skill[key].after.split(/[^0-9]/)[0]) <
            parseInt(skill[key].before.split(/[^0-9]/)[0]);
          return;
        }
        skill[key].isBuff =
          parseInt(skill[key].after.split(/[^0-9]/)[0]) >
  ...
  allData.runeChanges = await database.getAllRuneChangesForChampionIdAfterPatchId(
    championId,
    patchId
  );
  Object.keys(allData.runeChanges).forEach(patchKeys => {
    allData.runeChanges[patchKeys].forEach((rune, i) => {
      allData.runeChanges[patchKeys][i].changes = JSON.parse(
        allData.runeChanges[patchKeys][i].changes
      );
      Object.keys(rune.changes).forEach(key => {
        // key is COOLDOWN
        if (key == "name") return;
        let divider = rune.changes[key].indexOf(" => ");
        let before = rune.changes[key].substring(0, divider).split(/[^0-9]/);
        let after = rune.changes[key]
          .substring(divider + 4, rune.changes[key].length)
          .split(/[^0-9]/);
  ...
  allData.itemChanges = await database.getAllItemChangesForChampionIdAfterPatchId(
    championId,
    patchId
  );
  Object.keys(allData.itemChanges).forEach(patchKeys => {
    allData.itemChanges[patchKeys].forEach((rune, i) => {
      allData.itemChanges[patchKeys][i].changes = JSON.parse(
        allData.itemChanges[patchKeys][i].changes
      );
      ...
      item.isBuff =
        parseInt(after.split(/[^0-9]/)[0]) >
        parseInt(before.split(/[^0-9]/)[0]);
      if (
        item.attributes[0].attribute.includes("COOLDOWN") ||
        item.attributes[0].attribute.includes("COST")
      ) {
